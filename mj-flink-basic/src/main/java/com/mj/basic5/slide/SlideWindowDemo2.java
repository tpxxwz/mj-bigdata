...
public class SlideWindowDemo2 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        WatermarkStrategy<WaterMarkData> eventTimeStrategy =
                WatermarkStrategy.<WaterMarkData>forBoundedOutOfOrderness(Duration.ofSeconds(2))
                        .withTimestampAssigner((event, ts) -> event.getEventTime());
        parsedStream = parsedStream.assignTimestampsAndWatermarks(eventTimeStrategy);

        // 5. KeyBy用户ID
        KeyedStream<WaterMarkData, String> keyedStream = parsedStream.keyBy(WaterMarkData::getUserId);
        // 6. 定义窗口并处理窗口数据
        keyedStream.window(SlidingEventTimeWindows.of(Duration.ofSeconds(10),Duration.ofSeconds(5)))
