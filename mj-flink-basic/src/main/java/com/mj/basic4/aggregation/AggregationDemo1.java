...
public class AggregationDemo1 {
    public static void main(String[] args) throws Exception {
        ...
        DataStream<Order> maxByOrder = keyedOrders.maxBy("amount");

        // 4. 计算每个用户的最低单笔订单（完整记录）
        DataStream<Order> minByOrder = keyedOrders.minBy("amount");

        // 5. 计算每个用户的最高单笔订单（仅目标字段）
        DataStream<Order> maxOrder = keyedOrders.max("amount");

        // 4. 计算每个用户的最低单笔订单（仅目标字段）
        DataStream<Order> minOrder = keyedOrders.min("amount");

        // 输出结果到控制台（带描述前缀）
        //keyedOrders.print("keby结果");
